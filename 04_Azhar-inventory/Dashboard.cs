...
namespace Daftar_Pepustakaan
{
    public partial class Dashboard : Form
    {
        ...
        MySqlConnection connection = new MySqlConnection("server=localhost;user=root;password=;database=dinomart");

        public void Tampil(string valueToSearch)
        {
            MySqlCommand command = new MySqlCommand("select * from market where concat(id,nama_produk, harga, satuan, jumlah) like '%" + valueToSearch + "%'", connection);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            byte[] img = ms.ToArray();
            MySqlCommand command = new MySqlCommand("INSERT INTO market(nama_produk, harga, satuan, jumlah, foto) VALUES (@nama_produk, @harga, @satuan, @jumlah, @foto)", connection);
            ...
            command.Parameters.Add("@jumlah", MySqlDbType.VarChar).Value = textBox4.Text;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            pictureBox1.Image.Save(ms, pictureBox1.Image.RawFormat);
            byte[] img = ms.ToArray();
            MySqlCommand command = new MySqlCommand("UPDATE market SET nama_produk = @nama_produk, harga = @harga, satuan = @satuan, jumlah = @jumlah, foto = @foto WHERE id = @id", connection);
